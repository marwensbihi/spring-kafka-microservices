    ...
    private void createDummyBanksIfNeeded() {
        if (banqueRepository.count() == 0) {
            ...
            List<Banque> banks = Arrays.asList(
                    new Banque(null,"Amen Bank", 100000.0, 95000.0, companies.get(0)), // Amen Bank
                    new Banque(null, "BIAT",200000.0, 180000.0, companies.get(1)), // BIAT
                    new Banque(null, "STB",150000.0, 145000.0, companies.get(2)), // STB
                    new Banque(null, "ATB",180000.0, 175000.0, companies.get(3)), // ATB
                    new Banque(null, "BH",250000.0, 240000.0, companies.get(4)), // BH
                    new Banque(null, "BNA",120000.0, 110000.0, companies.get(5)), // BNA
                    new Banque(null, "UIB",90000.0, 85000.0, companies.get(6)),  // UIB
                    new Banque(null, "Zitouna Bank",50000.0, 45000.0, companies.get(7)),  // Zitouna Bank
                    new Banque(null, "Arab Tunisian Bank",300000.0, 290000.0, companies.get(8)), // Arab Tunisian Bank
                    new Banque(null, "Banque Centrale de Tunisie",350000.0, 345000.0, companies.get(9))  // Banque Centrale de Tunisie
            );
